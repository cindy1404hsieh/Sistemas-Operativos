...
expand_environ_var(char *arg)
{
	...
	if (arg[0] == '$') {
		...
		char *var_nombre = arg + 1;
		char *var_valor = getenv(var_nombre);

		if (var_valor) {
			size_t nuevo_len = strlen(var_valor) + 1;
			char *nuevo_arg = (char *) realloc(arg, nuevo_len);
			if (nuevo_arg) {
				strcpy(nuevo_arg, var_valor);
				return nuevo_arg;
			} else {
				return arg;
			}
