...
static int
sys_dec_priority(envid_t env_id)
{
	struct Env *env;
	int r = envid2env(env_id, &env, 0);
...
static int
sys_get_priority(envid_t env_id)
{
	struct Env *env;
	int r = envid2env(env_id, &env, 0);
	if (r || !env)
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_yield:
		sys_yield();  // No return
	case SYS_dec_priority:
		return sys_dec_priority(a1);
	case SYS_get_priority:
		return (int32_t) sys_get_priority(a1);
	default:
		return -E_INVAL;
	}
}
