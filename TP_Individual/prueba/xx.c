...
int main(int argc, char *argv[]) {
    ...
    if (pid == 0) {
        ...
        close(pipe[1]); // Cerramos el extremo de escritura de la tubería

        while (true) {
            int primo;
            
            if (read(pipe[0], &primo, sizeof primo) == 0) {
            ...
            if (pid == 0) {
                ...
                close(pipe[0]); // Cerramos el extremo de lectura de la tubería izquierda
                pipe[0] = pipe_derecha[0]; // Actualizamos la tubería izquierda
                close(pipe_derecha[1]); // Cerramos el extremo de escritura de la tubería derecha
            }
        }
        ...
        close(pipe[0]); // Cerramos el extremo de lectura de la tubería
